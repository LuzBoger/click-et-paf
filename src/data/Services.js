export const services = [
  {
    title: "La Gifle Thérapeutique",
    description:
      "Libérez vos tensions et vos blocages émotionnels avec une gifle administrée par un professionnel formé. Chaque gifle est accompagnée d'une phrase motivante personnalisée pour vous aider à vous recentrer et à vous ressourcer. Une expérience cathartique, sûre et contrôlée qui vous permet de libérer le stress tout en retrouvant équilibre et sérénité.",
    icon: "💆‍♂️",
  },
  {
    title: "La Gifle Cinématographique",
    description:
      "Plongez dans un univers cinématographique immersif où chaque gifle devient une scène inoubliable. Costumes authentiques, répliques cultes de films célèbres, et mise en scène spectaculaire vous feront sentir comme une star du cinéma. Une expérience ludique et mémorable pour les amateurs de cinéma et ceux qui rêvent de vivre une scène mythique.",
    icon: "🎭",
  },
  {
    title: "La Gifle Surprise",
    description:
      "Ajoutez un peu de piquant à votre journée avec une gifle surprise ! Vous ne savez jamais quand ni où cela se produira, mais une chose est sûre : ce moment sera inoubliable. Offrez-vous cette expérience pleine de fun et d’adrénaline, dans un environnement sécurisé, pour une montée d'excitation et de rires garantis.",
    icon: "🎉",
  },
  {
    title: "Stage",
    description:

      "Découvrez les secrets de l’art de donner et de recevoir une gifle avec notre stage interactif. Que ce soit pour libérer de la tension, exprimer des émotions ou simplement par plaisir, nos formateurs vous guideront à travers les différentes techniques et règles de sécurité pour une expérience maîtrisée et enrichissante. Vous repartirez avec des compétences pratiques et des souvenirs mémorables !",

    icon: "🎓",
  },
  {
    title: "La Gifle Sonore",
    description:
      "Transformez la gifle en une expérience multisensorielle avec La Gifle Sonore. Grâce à des micros haute définition et des effets sonores immersifs, chaque gifle résonne d’une manière spectaculaire. Idéale pour capturer des moments drôles et mémorables en vidéo, cette version amplifiée de la gifle vous fera vivre un son unique et intense.",
    icon: "🔊",
  },
  {
    title: "La Gifle Mémorable",
    description:
      "Faites l’expérience de la gifle sous un angle totalement nouveau avec La Gifle Mémorable. Grâce à la technologie du slow-motion et des effets dramatiques, chaque gifle devient un instant cinématographique. Immortalisez ce moment unique avec des images saisissantes et partagez-le avec vos proches pour qu’ils n’oublient jamais ce souvenir spectaculaire.",
    icon: "🎥",
  },
  {
    title: "La Gifle de Groupe",
    description:
      "Organisez un événement hors du commun avec vos amis, famille ou collègues grâce à La Gifle de Groupe. Que ce soit pour un enterrement de vie de garçon, un anniversaire ou un team-building, cette expérience collective ajoute de l’amusement et de l’adrénaline à n'importe quelle occasion. Rires et convivialité seront au rendez-vous, le tout dans un cadre sécurisé et avec un animateur pour guider l'événement.",
    icon: "🤾‍♂️",
  },
];
